...
const {getAppJson, test, getDirectories} = require("./testHelpers");
...
const testApp = appDirectory => {
	...
	test('should have a Class with same name as directory', () => {
		...
		const phpFileContent = readFileSync(`${appDirectory}/${appDirectory}.php`).toString();
	...
	test('should have the namespace based on the class name', () => {
		...
		return phpFileContent.indexOf(`namespace App\\SupportedApps\\${appDirectory}`) !== -1;
	})
};
